...
def todo_list():
    if 'username' in session:
        ...
        # Call the API to fetch the to-do list data
        response = requests.get(f'http://localhost:5000/api/todo-list/{username}')

        if response.status_code == 200:
            todos = response.json()['todos']
            return render_template('index.html', username=username, todos=todos)
        else:
            return render_template('index.html', username=username, todos=[])
